...
(defn count'
  ...
  {:level        :easy
   ...
   :implemented? true}
  ([coll]
   (loop [count-till-now 0 uncounted-coll coll]
     (if (empty? uncounted-coll)
         count-till-now
         (recur (inc count-till-now) (rest uncounted-coll))))))
...
(defn every?'
  ...
  {:level        :easy
   ...
   :implemented? true}
  ([pred coll]
   (loop [result true remaining-coll coll]
     (if (or (empty? remaining-coll) (false? result))
         result
         (recur (pred (first remaining-coll)) (rest remaining-coll))))))
...
(defn some?'
  ...
  {:level        :easy
   ...
   :implemented? true}
  ([pred coll]
   (loop [result false remaining-coll coll]
     (if (or (empty? remaining-coll) (true? result))
         result
         (recur (pred (first remaining-coll)) (rest remaining-coll))))))
...
(defn cross-product
  ...
  {:level        :easy
   ...
   :implemented? true}
  [seq1 seq2]
  (for [x seq1 y seq2 :while (not= x y)]
    [x y]
    ))
...
(defn split-comb
  ...
  {:level        :easy
   ...
   :implemented? true}
  [coll]
  (let [elements-count (count coll)
        interleaved-coll (apply interleave (split-at (int (/ elements-count 2)) coll))]
    (if (odd? elements-count)
        (concat interleaved-coll [(last coll)])
        interleaved-coll)
    )
  )
...
(defn index-of
  ...
  {:level        :easy
   ...
   :implemented? true}
  [coll n]
  (loop [index 0 remaining-coll coll]
    (cond
      (empty? remaining-coll) -1
      (= (first remaining-coll) n) index
      :else (recur (inc index) (rest remaining-coll)))))
